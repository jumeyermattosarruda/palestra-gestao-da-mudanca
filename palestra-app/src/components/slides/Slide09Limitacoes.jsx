...
const LIMITS = [
  { Icon: IconAlert, title: 'Alucinacoes', desc: 'A IA inventa fatos com muita confianca. Sempre valide informacoes criticas.' },
  { Icon: IconContext, title: 'Janela de Contexto', desc: 'A IA nao lembra conversas antigas. Cada sessao comeca do zero sem contexto persistente.' },
  { Icon: IconBias, title: 'Vies e Preconceito', desc: 'Modelos refletem vieses dos dados de treinamento. Output pode perpetuar estereotipos.' },
  { Icon: IconReason, title: 'Raciocinio Complexo', desc: 'Multi-step reasoning ainda falha em problemas que exigem senso comum ou julgamento humano.' },
  { Icon: IconData, title: 'Dados Atuais', desc: 'O knowledge cutoff impede acesso a eventos recentes. Para dados em tempo real, use tools especificas.' },
];
  ...
  return (
    <section id="slide-09" className="slide slide-dark" ref={ref}>
      <div className="w-full max-w-3xl px-6">
        <motion.h2 className="font-serif text-center mb-8"
        ...
        <div className="flex flex-col gap-5">
          {LIMITS.map(({ Icon, title, desc }, i) => (
            <motion.div key={i} className="flex items-start gap-5"
              initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: 0.15 * i, duration: 0.5 }}>
              <div className="mt-0.5 flex-shrink-0"><Icon /></div>
              <div>
                <p className="font-sans font-semibold mb-0.5" style={{ color: '#F0E6D6', fontSize: '1rem' }}>{title}</p>
                <p className="font-sans text-sm" style={{ color: '#C4A882', lineHeight: 1.5 }}>{desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
