    ...
    <section id="slide-03" className="slide slide-dark" ref={ref}>
      <div className="w-full max-w-5xl px-6">
        ...
        <div className="flex items-stretch gap-0 relative">
          ...
          >
            <p className="font-sans font-semibold text-sm tracking-widest uppercase mb-2" style={{ color: '#D4845A' }}>
              Gestão de Pessoas
            </p>
            {LEFT.map((item, i) => (
              <div key={i} className="flex items-start gap-4">
                ...
                <div>
                  <p className="font-sans font-semibold" style={{ color: '#F0E6D6', fontSize: '1rem' }}>{item.label}</p>
                  <p className="font-sans text-sm" style={{ color: '#C4A882' }}>{item.desc}</p>
                </div>
              </div>
            ))}
          </motion.div>
